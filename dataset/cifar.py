...
def get_cifar(args, norm=True):
    ...
    if name == 'cifar10':
        train_labeled_dataset = data_folder_main(root,
                                                 train_labeled_idxs,
                                                 train=True,
                                                 transform=norm_func)
        train_unlabeled_dataset = data_folder_main(root,
                                                   train_unlabeled_idxs,
                                                   train=True,
                                                   transform=norm_func,
                                                   return_idx=False)
        val_dataset = data_folder_main(root,
                                       val_idxs,
                                       train=True,
...
class TransformFixMatch(object):
    ...
    def __call__(self, x):
        ...
        if self.norm:
            return self.normalize(weak), self.normalize(
                strong), self.normalize(self.weak2(x))
        else:
            return weak, strong
...
class CIFAR100FIX(datasets.CIFAR100):
    def __init__(self,
        ...
        labels_unknown = self.targets[np.where(
            self.course_labels > num_super)[0]]
        labels_known = self.targets[np.where(
            self.course_labels <= num_super)[0]]
        ...
        assert len(
            np.where(self.targets == num_known)[0]) == len(labels_unknown)
...
def get_ood(dataset, id, test_only=False, image_size=None):
    ...
    if dataset == 'cifar10':
        ...
        test_set = datasets.CIFAR100(DATA_PATH,
                                     train=False,
                                     download=False,
                                     transform=test_transform)

    elif dataset == 'svhn':
        test_set = datasets.SVHN(DATA_PATH,
                                 split='test',
                                 download=True,
                                 transform=test_transform)
